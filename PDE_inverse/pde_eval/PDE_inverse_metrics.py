"""Evaluate FAPS inverse posterior metrics for PDE test sets.

This standalone evaluator shares one FAPS-DAPS implementation across Darcy,
Poisson, Helmholtz, and non-bounded Navier-Stokes.  PDE-specific differences
live in a small config table: paths, default device, observation count, and
...
class PDEConfig:
    ...
    def prior_fallback_dir(self) -> Path:
        return PDE_INVERSE_ROOT / "checkpoints" / "FAPS_prior" / "FNO"

    @property
    def local_prior_path(self) -> Path:
        label = "ns" if self.label == "ns_nonbounded" else self.label
        return self.prior_fallback_dir / f"{label}_fno_prior_{EPOCHS}.pt"
    ...
    def default_save_dir(self) -> Path:
        label = "ns" if self.label == "ns_nonbounded" else self.label
        return PDE_INVERSE_ROOT / "outputs" / "PDE_inverse_metrics" / label


PDE_CONFIGS = {
    "darcy": PDEConfig(
        label="darcy",
        prior_dir_name="darcy_input_prior_full",
    ...
    "poisson": PDEConfig(
        label="poisson",
        prior_dir_name="poisson_input_prior_full",
    ...
    "helmholtz": PDEConfig(
        label="helmholtz",
        prior_dir_name="helmholtz_input_prior_full",
    ...
    "ns_nonbounded": PDEConfig(
        label="ns_nonbounded",
        prior_dir_name="ns_nonbounded_input_prior_full",
...
def parse_args() -> tuple[argparse.Namespace, PDEConfig]:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate FAPS inverse posterior metrics over a PDE test set.",
        parents=[pre],
    )
...
def resolve_prior_path(cfg: PDEConfig, prior_path: Path | None, prior_dir: Path | None) -> Path:
    ...
    raise FileNotFoundError(f"Could not find OFM prior checkpoint in: {searched}")
...
def main() -> None:
    ...
    print(
        f"{cfg.label} FAPS test metrics config: "
