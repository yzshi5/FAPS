"""PDE inverse regression with a fixed-resolution UNet OFM prior and FNO surrogate."""
...
class VelocityUNetAdapter(torch.nn.Module):
    """Keep using UNet_cond while dropping dummy conditioning at UNet backbone."""
...
class FixedResolutionWhiteBaseDist:
    """Distribution adapter expected by the shared DAPS sampler."""
...
class OFMSurrogateDAPSSampler:
    """DAPS sampler with nonlinear likelihood through pretrained surrogate."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="PDE inverse problem with fixed-resolution UNet OFM prior + pretrained FNO surrogate."
    )
    ...
    parser.add_argument("--save-dir", type=Path, default=PDE_INVERSE_ROOT / "outputs" / "PDE_inverse_unet")
    ...
    parser.add_argument("--unet-channel-mult", type=str, default=None)
...
def build_config(args: argparse.Namespace) -> EvalConfig:
    channel_mult = None if args.unet_channel_mult in {None, "", "none", "None", "null"} else args.unet_channel_mult
    return EvalConfig(
        pde=args.pde,
...
def build_unet_model(cfg: EvalConfig) -> torch.nn.Module:
...
def build_prior_model(cfg: EvalConfig) -> OFMModel:
    if not cfg.prior_path.exists():
        raise FileNotFoundError(f"UNet OFM prior checkpoint not found: {cfg.prior_path}")
    print(f"Loading UNet OFM prior checkpoint from: {cfg.prior_path}")
    ...
    prior.gp.base_dist = FixedResolutionWhiteBaseDist(cfg.dims, device=cfg.device)
    return prior
...
def run_surrogate_regression_daps(
    G: OFMModel,
    forward_model: torch.nn.Module,
    dims: Sequence[int],
    pos_mask: Tensor,
    ...
    *,
    ...
    sampler = OFMSurrogateDAPSSampler(
        G=G,
...
def visualize_results(
    ...
    fig.suptitle(
        f"UNet-OFM {cfg.label} inverse regression | sample_idx={cfg.test_sample_idx} | "
...
def main() -> None:
    ...
    if cfg.dims != [128, 128]:
        raise ValueError(f"UNet prior evaluation only supports its training resolution, got {cfg.dims}.")


    print(
        "UNet-OFM regression config: "
    ...
    posterior = run_surrogate_regression_daps(
        G=prior,
        forward_model=forward_model,
        dims=cfg.dims,
        pos_mask=pos_mask,
