...
python "${PDE_ROOT}/pde_eval/PDE_inverse_metrics_unet.py" \
  --pde darcy \
  --prior-path "${PDE_ROOT}/checkpoints/FAPS_prior/UNet/darcy_unet_prior_100.pt" \
  ...
  --save-dir "${PDE_ROOT}/outputs/PDE_inverse_metrics_unet/darcy" \
  --device cuda:0 \
  --seed 300 \
  --start-idx 0 \
  --num-test 100 \
  ...
  --data-range 5.0 \
  --save-prefix darcy_unet_test_metrics
