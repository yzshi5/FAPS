...
python "${PDE_ROOT}/pde_eval/PDE_inverse_metrics.py" \
  --pde poisson \
  --prior-path "${PDE_ROOT}/checkpoints/FAPS_prior/FNO/poisson_fno_prior_100.pt" \
  ...
  --save-dir "${PDE_ROOT}/outputs/PDE_inverse_metrics/poisson" \
  --device cuda:0 \
  --seed 300 \
  --start-idx 0 \
  --num-test 100 \
  ...
  --data-range 5.0 \
  --save-prefix poisson_test_metrics
