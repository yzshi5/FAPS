...
python "${PDE_ROOT}/pde_eval/PDE_inverse_metrics_unet.py" \
  --pde ns_nonbounded \
  --prior-path "${PDE_ROOT}/checkpoints/FAPS_prior/UNet/ns_unet_prior_100.pt" \
  --forward-ckpt "${PDE_ROOT}/checkpoints/PDE_surrogate/ns_forward.pt" \
  --test-data-path "${PDE_ROOT}/datasets/ns/ns_pde_test_200.npy" \
  --save-dir "${PDE_ROOT}/outputs/PDE_inverse_metrics_unet/ns" \
  --device cuda:0 \
  --seed 300 \
  --start-idx 0 \
  --num-test 100 \
  ...
  --data-range 5.0 \
  --save-prefix ns_nonbounded_unet_test_metrics
