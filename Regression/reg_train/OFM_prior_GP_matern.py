"""Train and evaluate OFM on 1D GP prior data."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and/or evaluate OFM on 1D GP prior data."
    )
    parser.add_argument(
        "--save-dir",
        type=Path,
        default=REGRESSION_ROOT / "outputs" / "OFM" / "GP_matern",
    )
    parser.add_argument(
        "--checkpoint-name",
        type=str,
        default=None,
        help="Optional checkpoint filename under --save-dir, e.g. GP_matern_epoch_500.pt.",
    )
    parser.add_argument(
        "--figure-dir",
        type=Path,
...
def train_ofm(loader_tr: DataLoader, cfg: TrainConfig) -> None:
    ...
    ofm.train(
        ...
        saved_model=True,
        loss_filename=None,
        loss_log_filename="GP_matern_loss.csv",
    )
