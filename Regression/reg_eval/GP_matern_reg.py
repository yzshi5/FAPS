...
from faps_utils.true_gaussian_process import true_GPPrior
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate OFM-FAPS masked regression with a GP Matern prior.")
    ...
    parser.add_argument("--results-dir", type=Path, default=REGRESSION_ROOT / "outputs" / "Regression_results" / "GP_matern_reg")
    ...
    parser.add_argument("--n-x", type=int, default=1024)
    parser.add_argument("--gp-length-scale", type=float, default=0.3)
    parser.add_argument("--gp-variance", type=float, default=1.0)
    parser.add_argument("--gp-nu", type=float, default=1.5)
    parser.add_argument("--base-case-nx", type=int, default=128)
    parser.add_argument("--case-seed", type=int, default=100)
...
def build_config(args: argparse.Namespace) -> EvalConfig:
    cfg = EvalConfig(
        ...
        gp_variance=args.gp_variance,
        gp_nu=args.gp_nu,
        base_case_nx=args.base_case_nx,
    ...
    if cfg.n_x not in (128, 512, 1024):
        raise ValueError(f"n_x must be one of (128, 512, 1024), got {cfg.n_x}.")
    return cfg
...
def resolve_checkpoint_path(cfg: EvalConfig) -> Path:
    ...
    candidates = [
        ...
        cfg.model_dir / f"GP_matern_epoch_{cfg.epochs}.pt",
        cfg.flat_checkpoint_dir / f"GP_matern_epoch_{cfg.epochs}.pt",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise FileNotFoundError(
        "Could not find Matern prior checkpoint. Checked: "
...
def _interp_1d_curve(values: np.ndarray, n_target: int) -> np.ndarray:
    """Linear interpolation from source grid [0,1] to target grid [0,1]."""
    n_src = values.shape[-1]
    x_src = np.linspace(0.0, 1.0, n_src, dtype=np.float64)
    x_tgt = np.linspace(0.0, 1.0, n_target, dtype=np.float64)
    return np.interp(x_tgt, x_src, values.astype(np.float64)).astype(np.float32)


def build_shared_regression_case(
    n_query: int,
    noise_var: float,
    cfg: EvalConfig,
) -> Tuple[Tensor, Tensor, Tensor, np.ndarray, np.ndarray]:
    """
    Build one fixed synthetic case shared across query sizes.

    Fixed across n_query:
      1) Ground-truth source sample is drawn at cfg.base_case_nx.
      2) Observation coordinates are selected on the cfg.base_case_nx grid.
      3) Observation noise vector is fixed (same values reused).
    """
    np_state = np.random.get_state()
    torch_state = torch.get_rng_state()
    np.random.seed(cfg.case_seed)
    torch.manual_seed(cfg.case_seed)

    gp_base = true_GPPrior(
        lengthscale=cfg.gp_length_scale,
        var=cfg.gp_variance,
        nu=cfg.gp_nu,
        device="cpu",
        dims=[cfg.base_case_nx],
    )
    x_test_base = gp_base.sample_from_prior(dims=[cfg.base_case_nx], n_samples=cfg.n_test)
    u_obs_base = x_test_base[cfg.test_sample_idx : cfg.test_sample_idx + 1, 0:1].detach().cpu().numpy()
    u_obs_base_1d = u_obs_base.reshape(-1)  # [base_case_nx]

    # Fixed observation coordinates on the base (128) grid.
    base_obs_idx = np.random.choice(cfg.base_case_nx, cfg.n_obs, replace=False)
    base_obs_idx.sort()
    obs_x = base_obs_idx.astype(np.float64) / max(cfg.base_case_nx - 1, 1)

    # Fixed noise vector shared by all n_query choices.
    fixed_noise = (
        torch.randn(1, 1, cfg.n_obs, dtype=torch.float32) * math.sqrt(float(noise_var))
    ).detach().cpu().numpy().reshape(-1)

    # Query-resolution ground truth from the same 128-res source sample.
    u_query_1d = _interp_1d_curve(u_obs_base_1d, n_query)
    u_obs_full_t = torch.from_numpy(u_query_1d).reshape(1, 1, n_query)

    # Map fixed observation coordinates to the current query grid.
    query_obs_idx = np.rint(obs_x * (n_query - 1)).astype(int)
    query_obs_idx = np.clip(query_obs_idx, 0, n_query - 1)
    query_obs_idx = np.unique(query_obs_idx)
    if query_obs_idx.size != cfg.n_obs:
        raise RuntimeError(
            f"Observation index collision for n_query={n_query}: "
            f"expected {cfg.n_obs}, got {query_obs_idx.size}."
        )

    pos_mask_t = torch.zeros(n_query, dtype=torch.bool)
    pos_mask_t[torch.from_numpy(query_obs_idx)] = True

    clean_obs = u_obs_full_t[:, :, pos_mask_t].detach().cpu().numpy().reshape(-1)
    noisy_obs = clean_obs + fixed_noise
    u_obs_part_t = torch.from_numpy(noisy_obs.reshape(1, 1, -1)).to(torch.float32)

    np.random.set_state(np_state)
    torch.set_rng_state(torch_state)
    return u_obs_full_t, u_obs_part_t, pos_mask_t, base_obs_idx, query_obs_idx
...
class OFMMaskedFAPSSampler:
    ...
    def __init__(
        ...
        noise_level: float,
        *,
        gp_ref: Optional[true_GPPrior] = None,
        tau: float = 1.0,
...
def ground_truth_gp_posterior_reference(
    ...
    """
    Compute the exact GP posterior reference under the data-generating Matern GP prior.
    """
    ...
    kernel = cfg.gp_variance * Matern(length_scale=cfg.gp_length_scale, nu=cfg.gp_nu)
    ...
    gp.fit(x_train, obs_values)

    gp_mean, gp_std = gp.predict(x_grid, return_std=True)
    gp_samples = gp.sample_y(x_grid, n_samples=n_draws, random_state=22).T
    return {"mean": gp_mean, "std": gp_std, "samples": gp_samples}
...
def visualize_masked_regression(
    ...
    with plt.rc_context(plot_style):
        ...
        x_axis_plot = np.linspace(0.0, 1.0, samples_np.shape[-1])
        obs_x_plot = obs_idx / max(samples_np.shape[-1] - 1, 1)


        fig, ax = plt.subplots(1, 1, figsize=(8, 4))
        for idx in draw_idx:
            ax.plot(x_axis_plot, samples_np[idx], color="#FF4500", alpha=0.4, linewidth=1.1)
        ax.plot(x_axis_plot, post_mean, color="black", linewidth=2.6, label="Posterior mean")
        ax.fill_between(
            x_axis_plot,
        ...
        ax.fill_between(
            x_axis_plot,
            gp_ref["mean"] - 2.0 * gp_ref["std"],
            gp_ref["mean"] + 2.0 * gp_ref["std"],
            color="#1f77b4",
            alpha=0.34,
            label="True GP mean +/- 2 std",
        )
        ax.plot(x_axis_plot, true_curve, color="k", linewidth=2.3, label="Ground truth")
        ax.scatter(
            obs_x_plot,
            obs_values,
            color="#C44E52",
        ...
        ax.set_title(f"Query size = {cfg.n_x}")
        if cfg.n_x == 1024:
...
def main() -> None:
    ...
    u_obs_full, u_obs_part, pos_mask, base_obs_idx, query_obs_idx = build_shared_regression_case(
        n_query=cfg.n_x,
        noise_var=cfg.noise_level,
        cfg=cfg,
    )
    print(
        f"Shared case config: base_nx={cfg.base_case_nx}, query_nx={cfg.n_x}, "
        f"case_seed={cfg.case_seed}, test_idx={cfg.test_sample_idx}"
    )
    print(f"Base obs idx ({cfg.base_case_nx}-grid): {base_obs_idx.tolist()}")
    print(f"Query obs idx ({cfg.n_x}-grid): {query_obs_idx.tolist()}")

    u_obs_part = u_obs_part.to(cfg.device)
